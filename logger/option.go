...
type config struct {
	...
	traceEnable   bool
	logPusher     sdklog.LoggerProvider
}
...
// WithTraceEnable 启用trace
func WithTraceEnable(enableTrace bool) Option {
	return option(func(cfg *config) {
		cfg.traceEnable = enableTrace
	})
}
...
// WithLogPusher 添加日志推送器
func WithLogPusher(logPusher sdklog.LoggerProvider) Option {
	return option(func(cfg *config) {
		cfg.logPusher = logPusher
	})
}
